...
// admin SPA PUTs directly to a presigned S3 URL and this router never exists.
//
...
export function uploadsRoutes(store: LocalObjectStore, jwtSecret: string) {
  ...
  // Both transports are admin-only: uploads come exclusively from the admin SPA.
  r.use('*', requireAuth(jwtSecret), requireAdmin);
